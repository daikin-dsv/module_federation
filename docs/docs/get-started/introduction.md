...
- **[Start fresh](/get-started/rad-template/)** – Use the RAD Template when you are bootstrapping a new full-stack experience. It brings Next.js, DDS wiring, auth, testing, CI/CD, and AI-friendly docs so you can ship quickly with consistent patterns. Create the repository under the `daikin-dsv` GitHub organization to use our deployment pipeline.
- **[Layer on shared chrome](/get-started/layouts/)** – Use the Layout package when you need universally-required chrome (auth, header, nav, footer) inside an existing application. Import once, register the custom elements globally, and keep your current framework.
- **[Add app-specific UI](/get-started/widgets/)** – Reach for Widgets when you want reusable alarms, confirmation flows, gauges, tables, or Databricks embeds. They are standalone elements you can sprinkle into any host without adopting the full template.
...
- **[Developer sandbox & AI helpers](/get-started/rad-template/sandbox/)** (plus [Building with AI](/get-started/rad-template/AI/)) – Guides for spinning up GitHub Codespaces, sharing forwarded ports, and letting Bolt or Codespaces chat agents consume the project briefs safely.
- **[Layouts](/get-started/layouts/)** – Lit-based `<auth-provider>`, `<app-header>`, `<nav-menu>`, `<user-profile>`, and `<app-footer>` components packaged with DDS tokens, Tailwind-aware styles, and Keycloak helpers. Import them directly into any repo to standardize chrome without touching your routing stack.
...
- **[Samples](/samples/AI-Generated%20Apps/energy-management/)** – Screenshot-heavy references for layouts, widgets, and AI-generated dashboards (Energy Management, Fault Detection, SOX Audit) that showcase the end-state experiences these packages enable.
