...
2. Configure environment:
    ...
    npx auth secret
    ```
    `npx auth secret` generates the required `AUTH_SECRET` value. Set the required variables:
    ```
    RAD_URL=
    AUTH_SECRET=
    AUTH_KEYCLOAK_ID=
    ...
    PUBLIC_DOCS=false
    ```
    Set `BYPASS_AUTH=true` for local development to skip `Keycloak`. Set `PUBLIC_DOCS=false` to lock the documentation behind SSO.
3. Run the app (available at `http://localhost:3006`):
    ```bash
    npm run dev
    ```
